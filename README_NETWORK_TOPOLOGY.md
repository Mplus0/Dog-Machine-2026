...
`wlan0` 当前地址由手机热点 DHCP 分配，可能随网卡、租约或热点配置发生变化。本文记录的是当前实测地址，不代表固定静态地址。
...
感知主机 Jetson / Ubuntu 20.04 / ROS Noetic
  wlan0: 172.20.10.2/28，DHCP
  wlan0: 连接 iPhone-hotspot，负责 SSH、Git、Internet 和默认路由
...
| 设备/接口                | IP 或名称                  | 说明                                                    |
| ------------------------ | -------------------------- | ------------------------------------------------------- |
| 手机热点网关             | `172.20.10.1`              | 感知主机 `wlan0` 的默认路由网关                         |
| 开发 WiFi/手机热点       | `iPhone-hotspot`           | 开发机和感知主机管理网络                                |
| 感知主机 `wlan0`         | `172.20.10.2/28`           | 当前 DHCP 地址，负责 SSH、Git、Internet 和默认路由      |
...
开发机连接 `iPhone-hotspot` 后，使用感知主机 `wlan0` 当前 DHCP 地址登录：

```bash
ssh ysc@172.20.10.2
```

`172.20.10.2` 是当前 DHCP 租约。地址变化时，先在手机热点客户端列表中查询，或在感知主机执行：
...
default via 172.20.10.1 dev wlan0 proto dhcp metric 601
...
172.20.10.0/28 dev wlan0 proto kernel scope link src 172.20.10.2 metric 601
```

含义：

- 访问 Internet 和手机热点网段走 `wlan0`，当前默认网关为 `172.20.10.1`。
...
```text
wlan0 -> iPhone-hotspot
  IP: 172.20.10.2/28（DHCP 当前租约）
  gateway: 172.20.10.1
...
工具在感知主机运行时，开发机连接 `iPhone-hotspot` 后，使用感知主机 `wlan0` 当前 DHCP 地址访问：

```text
D435i 彩色预览:       http://172.20.10.2:8080
广角相机预览:         http://172.20.10.2:8081
ROS 导航调试看板:     http://172.20.10.2:8082
```

`172.20.10.2` 是当前 DHCP 租约，不是固定地址。无法访问时，先在感知主机执行：
