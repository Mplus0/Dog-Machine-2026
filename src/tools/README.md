...
```text
开发 WiFi/手机热点：iPhone-hotspot，网关 172.20.10.1
感知主机 wlan0：连接 iPhone-hotspot，当前 DHCP 地址 172.20.10.2/28，默认路由
...
本目录工具默认假设在感知主机上运行。浏览器访问网页工具时，开发机使用 `http://172.20.10.2:<port>`；该地址是当前 DHCP 租约，变化时先查询 `wlan0`。感知主机访问运动主机时使用 `192.168.1.120`。
...
```yaml
robot_hotspot_ip: 172.20.10.2
developer_wifi_ssid: iPhone-hotspot
perception_wifi_adapter: 368b:8d85
...
如果感知主机没有自动连接 `iPhone-hotspot`，但电脑能够连接机器狗 AP，可通过运动主机跳板登录：
