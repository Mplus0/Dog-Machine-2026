...
感知主机 wlan0：连接 Bad_Puppy，固定地址 192.168.31.192/24，默认路由
感知主机 wlan1：连接机器狗 AP，192.168.2.92/24，ipv4.never-default=yes
...
本目录工具默认假设在感知主机上运行。浏览器访问网页工具时，开发机使用 `http://192.168.31.192:<port>`；该地址是当前固定地址，连接异常时先检查 `wlan0`。感知主机访问运动主机时使用 `192.168.1.120`。
...
```yaml
robot_hotspot_ip: 192.168.31.192
