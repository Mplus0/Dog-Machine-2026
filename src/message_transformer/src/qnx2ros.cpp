...
public:
  QNX2ROS(ros::NodeHandle nh, int server_port){
    ...
    robot_basic_state_pub_ = nh.advertise<std_msgs::Int32>("/lite3/robot_basic_state", 1);
    robot_gait_state_pub_ = nh.advertise<std_msgs::Int32>("/lite3/robot_gait_state", 1);
    robot_motion_state_pub_ = nh.advertise<std_msgs::Int32>("/lite3/robot_motion_state", 1);
    robot_battery_level_pub_ = nh.advertise<std_msgs::Float64>("/lite3/battery_level", 1);
    robot_need_move_pub_ = nh.advertise<std_msgs::Bool>("/lite3/is_robot_need_move", 1);
  }
  ...
  void ParseRobotStateFrame(){
    ...
    if (dr->code == 2305) 
    {
      ...
      std_msgs::Int32 robot_basic_state_msg;
      robot_basic_state_msg.data = robot_state->robot_basic_state;
      robot_basic_state_pub_.publish(robot_basic_state_msg);

      std_msgs::Int32 robot_gait_state_msg;
      robot_gait_state_msg.data = robot_state->robot_gait_state;
      robot_gait_state_pub_.publish(robot_gait_state_msg);

      std_msgs::Int32 robot_motion_state_msg;
      robot_motion_state_msg.data = robot_state->robot_motion_state;
      robot_motion_state_pub_.publish(robot_motion_state_msg);

      std_msgs::Float64 battery_level_msg;
      battery_level_msg.data = robot_state->battery_level;
      robot_battery_level_pub_.publish(battery_level_msg);

      std_msgs::Bool need_move_msg;
      need_move_msg.data = robot_state->is_robot_need_move;
      robot_need_move_pub_.publish(need_move_msg);
